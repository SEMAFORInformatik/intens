...
#if !defined(JOB_STACK_DATA_INTEGER_INCLUDED_H)
#define JOB_STACK_DATA_INTEGER_INCLUDED_H

#include "job/JobStackData.h"

class JobStackDataInteger : public JobStackData
{
/*=============================================================================*/
...
public:
  ...
  JobStackDataInteger( int value )
    : JobStackData( true )
    , m_value( value ){
  }
  virtual ~JobStackDataInteger(){}
...
public:
  ...
  */
  DataDictionary::DataType getDataType() { return DataDictionary::type_Integer; }

  /** Diese Funktion erstellt eine Kopie des Objekts.
      @return Pointer auf neues Objekt.
  */
  virtual JobStackDataInteger *clone();
  ...
  /** Diese Funktion liefert Imaginär-Teil des Werts als double. Falls Der Wert
      ...
      @return True: => Der gewünschte Wert ist gültig.
   */
  virtual bool getImagValue( double &d ) { d = 0.0; return isValid(); }
