...
\index{CLASS@\CLASS!function statement}
\begin{tabularx}{\textwidth}{l|X}
...
\ASSIGNCONSISTENCY & Assign the (value of the) second argument to the first. \newline
                    ...
                    Dependency checking is otherwise only done when a value is entered in the gui, but not
                    with a normal assignment (a = b;). \\
\CLASS            & Set or change the class property of an ui element. \newline
...
\index{TITLE@\TITLE!function statement (set)}
\index{EXPAND@\EXPAND!function statement (set)}
...
\begin{boxedminipage}[t]{0.8\linewidth}
\begin{intens}
