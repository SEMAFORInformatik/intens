...
\hline
\AUTOLEVEL        & Hides structure-identifiers of structs with a single child. \\
\HIDEEMPTYFOLDERS & Does not display any structure-identifiers without entries. \\
\OPENLEVELS       & Opens (expands) all folders up to and including tree-level 'integer'.
                    Closes (collapses) all folders from tree-level 'integer' onwards. \\
\FIRSTLEVEL       & Does not display structure-identifiers before tree-level 'integer'. \\
\LASTLEVEL        & Hides everything from tree-level 'integer' onwards. \\
\end{tabularx}
