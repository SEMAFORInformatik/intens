...
- Download the DEB-File and install Intens:
  #+begin_example
  sudo apt-get install ./semafor-intens-5.3.4dev-ubuntu-24.04-x86-64.deb
  #+end_example
  This will install the files in /usr/local/intens.
- Add the intens bin directory to your PATH:
  #+begin_example
  PATH=/usr/local/intens/5.3.4dev/bin:$PATH
  #+end_example
...
- If you're working with Emacs add the following lines to your
    emacs config:
  #+begin_example
    (add-to-list 'load-path "/usr/local/intens/5.3.4dev/etc")
