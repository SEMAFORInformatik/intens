...
#if !defined(__DP_DATA_ALTER_ASSIGN_DATAITEM__)
#define __DP_DATA_ALTER_ASSIGN_DATAITEM__

#include "datapool/DataAlterFunctor.h"

class DataItem;

/** Mit einem DataAlterAssignDataItem-Objekt wird einem DataItem der Inhalt eines
    anderen DataItem zugewiesen.
    @author Copyright (C) 2015  SEMAFOR Informatik & Energie AG, Basel, Switzerland
 */
class DataAlterAssignDataItem : public DataAlterFunctor
{
/*=============================================================================*/
...
public:
  /** Konstruktor
      @param item Pointer auf das zuzuweisende DataItem
   */
  DataAlterAssignDataItem( const DataItem *item );

  /** Destruktor
      \todo Die Beschreibung der Funktion ~DataAlterAssignDataItem fehlt
   */
  virtual ~DataAlterAssignDataItem();
...
/*=============================================================================*/
protected:
  /** Die Funktion zeigt an, ob das Item gültig ist.
      @return true -> DataItem für den assign ist vorhanden.
   */
  bool isValid() const;
