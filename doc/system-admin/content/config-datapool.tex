...
\NOCOLORBIT      & The eight available color bits
...
\LABEL{} = ...   & Set the item's \LABEL{} which can be used by the \UIMANAGER{} and \STREAMER.
                   The \LABEL{} is set to the item's name by default if no \LABEL{} is provided.
...
\PATTERN         & A regular expression that defines the possible input values. \newline
                   It has to match the entire expression. \newline
                   Only implemented for \STRING. \\+
...
\RANGE           & Minimal and maximal values for a \SLIDER. \newline
                   Or the range of possible input values for \REAL{} or \INTEGER{} values. \\
