...
\item[\texttt{--}help] prints the list of INTENS commandline options and exists. \\
  No description file is needed.
%
\item[\texttt{--}withTargetStreamInfo <argument>]
  When INTENS detects a dependency violation between input and output values,
  if automatically clears the output (or target) values (\AUTOCLEARDEPENDENCIES) or
  it shows a confirm dialog: \\ \\
  The results are not consistent anymore. \\
  Do you want to delete them? \\
  Yes / No \\  \\
  With this commandline option, the dialog also shows the name(s) of the
  target stream(s) (output values) involved.
%
...
%
\item[\texttt{--}withKeypadDecimalPoint <argument>]
  TODO
%
...
%
\item[\texttt{--}withoutRangeCheck <argument>]
  TODO
%
\item[\texttt{--}test <argument>] enables TestMode:
  ...
  \end{itemize}
%
\item[\texttt{--}testmode] enables TestMode.
%
\item[\texttt{--}noInitFunc] Don't run the INIT function.
%
\item[\texttt{--}replyPort <argument>]
  An INTENS application can run as a desktop program or in the browser (webtens).
  When run in the browser, INTENS must be started with this commandline option.
  INTENS will listen on this tcp port for zmq requests. Webtens sends all
  user inputs to INTENS as zmq requests, on this port. \\

  This can also be used for testing an INTENS application: a test software, i.E.
  python behave, can set or get values, call functions, etc. using the same
...
\item[\texttt{--}defaultMessageQueueDependencies <argument>]
  ...
  \begin{itemize}
  \item 1 or ``true'' (=default): the \STREAM{}s are used with dependencies by default
  \item 0 or ``false'': the \STREAM{}s are not used with dependencies by default
  \end{itemize}
  Note: the reason for this commandline option was the introduction of
  \MESSAGEQUEUE{} \REQUEST{} dependencies. This option allows existing applications
  to behave as before, without changing the application code: \newline
  \texttt{--defaultMessageQueueDependencies=0}.
%
\item[\texttt{--}oauth <argument>]
  TODO
%
\item[\texttt{--}oauthAccessTokenUrl <argument>]
  TODO
%
\item[\texttt{--}opentelemetryMetadata]
  TODO
%
\item[\texttt{--}lspWorker]
  TODO
%
