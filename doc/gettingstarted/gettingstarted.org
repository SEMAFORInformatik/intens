...
can be found in section [[*Database Service Installation]].
...
1. Create a new project directory:
  ...
  $ cd intens-training
  #+end_example
2. Download the DB service from https://owncloud.semafor.ch/s/B1RWj71nvPNO89t/download (100 MB)
  and unpack the file:
  #+begin_example
...
A user must have an active group. When a component is created,
...
#+end_example
- Create a virtual environment for python: This is used to manage python packages
  for different projects. There are several options. We strongly recommend using [[https://docs.astral.sh/uv]]
#+begin_example
~/intens-training $ uv init --bare
~/intens-training $ uv add Mako
#+end_example
  The above commands create  the file ~pyproject.toml~ ``training'' and add the python package
  mako as a dependency.
...
📁 etc
   📁 motor
      ...
      📄 motor_operator.inc
#+end_example
- Open the file ~etc/motor/motor_datapool.inc~ and include the data items in the Motor STRUCT block:
#+begin_src intens
...
#+begin_example
(training) ~/intens-training/scim $ cd api-gateway && uv sync
#+end_example
...
#+END_EXAMPLE
*** Installation
Download from [https://allurereport.org/docs/v2/install] (Version 2), unpack the file and
add the bin directory of the unpacked files to your ~PATH~ variable.
...
(See [[https://behave.readthedocs.io/en/latest/tutorial/#environmental-controls][Environmental Controls]] for more information.)
...
*** Running the tests
#+begin_example
(training) ~/intens-training $ cd scim/testing && uv sync
...
*** Desktop login with DB-dashboard

The login to the desktop application can alternatively be delegated to the db service dashboard.
To do that, you just have to pass the URL of the dashboard as a launch option:
#+begin_example
  --dashboardURL http://example.com/workbook
#+end_example

With this option, your app will open the dashboard when it needs you to log in.
The dashboard will then prompt you to open the ~intens://~ url handler which will then log you in on your desktop app.
If the dashboard doesn't trigger the login automatically, there's a button "Login to local INTENS" on the site to do it manually. Then press the
button "Open Intens URI Handler" to proceed.

#+caption: Intens login handling with the DB service dashboard
#+attr_html: :width 500px :align center
[[./images/dashboard-intens-login.png]]

*** OAuth in DB-dashboard

If you set the Workbook to use OAuth, you will also need to set up the dashboard accordingly.
The dashboard is a public oauth client which must be configured in the DB service (application yaml):

#+begin_example
app.dashboard.authorizationEndpoint=https://keycloak.example.com/realms/semafor/protocol/openid-connect/auth
app.dashboard.tokenEndpoint=https://keycloak.example.com/realms/semafor/protocol/openid-connect/token
app.dashboard.clientId=dashboard
app.dashboard.dashboardUrl=https://webtens.semafor.ch/workbook
#+end_example
...
Now install the Python packages from the pyproject.toml and run the Behave test
#+begin_example
(training) ~/intens-training/testing/webtesting $ uv sync
...
to a SQL or NOSQL database. The data model  includes 3 major  types (groups)
of entities:
...
** Change Management and Approval States
Components and variants have an approval state to ensure unwanted modifications. Only components and variants with one of the approval state =Experimental=, =In Preparation=, or =Shared= are mutable. Components and variants that are =Approved=, =Tendered=, =Tested= or =Obsolete= are inmutable. The workbook service will create a copy with an increased revision number on saving an inmutable object in case of any modifcation and its approval state will be reset to =Experimental=. The valid state transitions can be configured in the file =application.yml=. 
#+CAPTION: Approval States
#+NAME:   fig:approval-states
#+attr_html: :width 500px :align center
[[file:./images/approval-states.png]]

** Access Permissions
The access permissions depend on the approval states and are defined in the
file =application.yml= and =AccessRules.drl=:
| Permission | Description      | Configuration   |
|------------+------------------+-----------------|
| R          | read (get by id) |                 |
| M          | modify (update)  | =AccessRules.drl= |
| L          | list (select)    | =application.yml= |

*** Components
| Approval State | Group | Others |
|----------------+-------+--------|
| experimental   | R - - | R - -  |
| inPreparation  | R M L | R - -  |
| shared         | R M L | R M -  |
| approved       | R - L | R - -  |
| obsolete       | R - L | R - -  |
...
- access permissions for list requests (L);
#+begin_src yaml
AccessFilter:
  active: true

component:
# defaultApprovalStateRev0: experimental
  defaultApprovalState: experimental
  approval:
    group:
      experimental: false
      inPreparation: true
...
validStateTransitions:
  Experimental: Experimental,InPreparation,Shared,Approved,Obsolete
  InPreparation: InPreparation,Shared,Approved,Obsolete
  Shared: Shared,Approved,Obsolete
  Approved: Approved,Tested,Tendered,Obsolete
  Obsolete: Obsolete
  Tested: Tested,Tendered,Obsolete
  Tendered: Tendered,Obsolete

projectStatusTransitions:
  Global: Local,Global,Delivered,Obsolete
  Local: Local,Division,Global,Delivered,Obsolete
  Obsolete: Obsolete
  Division: Division,Local,Global,Delivered,Obsolete
  Delivered: Delivered
#+end_src
* Appendix
...
*** Intens
Download and unpack [[https://github.com/semaforinformatik/intens/releases/semafor-intens-6.0.7-win64.zip]]
and add the Bin directory to your PATH variable:
#+begin_example
PATH=/c/semafor/intens/6.0.9/bin:$PATH
#+end_example
...
- Download the DEB-File from https://github.com/SEMAFORInformatik/intens/releases/ and install Intens:
  #+begin_example
  sudo apt-get install ./semafor-intens-6.0.9-ubuntu-24.04-x86-64.deb
  #+end_example
  This will install the files in /usr/local/intens.
- Add the intens bin directory to your PATH:
  #+begin_example
  PATH=/usr/local/intens/6.0.9/bin:$PATH
  #+end_example
...
- If you're working with Emacs add the following lines to your
    emacs config:
  #+begin_example
    (add-to-list 'load-path "/usr/local/intens/6.0.9/etc")
