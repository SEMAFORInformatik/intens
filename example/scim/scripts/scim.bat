...
set TOP_DIR=%cd%
set USER_DIR=%USERPROFILE%\scim
set LOG_DIR=%USER_DIR%\log
set STARTUP_LOG=%LOG_DIR%\scim.startup
...
if debug == 1 (
    echo "Starting SCIM" >> %STARTUP_LOG%
)
...
intens.exe ^
    ...
    --resfile %TOP_DIR%\etc\scim.ini ^
    %TOP_DIR%\etc\scim.des > %LOG_DIR%\scim.log 2> %LOG_DIR%\scim.err
